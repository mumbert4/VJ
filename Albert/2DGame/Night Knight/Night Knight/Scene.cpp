...
void Scene::render()
{
	...
	enemy_pos = (sin(currentTime / 500.f) / 10);
	enemies[0]->setPosition(glm::vec2((25 * map->getTileSize()) * (enemy_pos + 1), 5 * map->getTileSize()));
	enemies[0]->render();
	enemies[1]->setPosition(glm::vec2((10 * map->getTileSize()) * (enemy_pos + 1), 5 * map->getTileSize()));
	enemies[1]->render();
}
